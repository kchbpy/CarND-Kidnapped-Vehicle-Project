...
@param (x_ob,y_ob) x and y coordinates of observation
@param (x_m,y_m) x and y coordinates of the assosiate landmark
@param std[] x and y thegma
@output Multivariate-Gaussian probability
*/

inline double MG_prob(double x_ob, double y_ob, double x_m, double y_m, double std[]){
	double gauss_norm = (1/(2 * M_PI * std[0]* std[1]));
	double expon= ((x_ob-x_m)*(x_ob-x_m))/(2*std[0]*std[0])+
		((y_ob-y_m)*(y_ob-y_m))/(2 * std[1]*std[1]);
	double weight= gauss_norm * exp(-expon);
	return weight;
}
